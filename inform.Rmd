...
This document presents the results of the analysis of the "Health, Nutrition and Population Statistics" dataset hosted by the World Bank Group. It was compiled from an RMarkdown file with the code which is also available. A Shiny App showing the discussed topic and other interesting visualizations can be accessed here. 

The idea is to analyse the progress towards the resolution of some health issues focusing in gender differences. I will show how some female/male rates have improved and . I will study correlations with education in some cases.

## Importing and tidying the data

First of all I had to import the data, I did this with the `readr` package.
...
**Decision:** I chose to use the `tidyverse` environment instead of the `data.table` package because the computational needs were not too high and code is more readable with `dplyr`. 
